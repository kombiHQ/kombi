...
class VendorXPngPlate(PngInfoCrate):
    """
    Implements a custom vendor "X" png plate infoCrate.
    ...
    def __init__(self, *args, **kwargs):
        """
        Create a vendor "X" png plate infoCrate.
        """
    ...
    def test(cls, data, parentInfoCrate=None):
        """
        Return a boolean telling if the input data can become a vendor "x" png plate infoCrate.
        """
        # perform the tests for the base classes
        if super(VendorXPngPlate, cls).test(data, parentInfoCrate):
