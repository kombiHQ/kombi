...
Kombi is focused in describing and performing evaluations. These evalutions can be used to wrap from executables to wrap arbitrary python implementations. It works by providing an API that simplifies the process of describing nested evaluations, passing data across evaluations, handling options/settings for the evaluations, dealing with path manipulation/file system operations, splitting the processing and, providing interoperability across different applications.
