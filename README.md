...
Kombi is focused on processing data across different applications and libraries, such as during image/video processing, ingesting files, versioning data, (etc). This is done by providing tasks which are used to wrap the execution of applications/libraries in a common generic way that simplifies the process of defining specifications and, providing interoperability across different applications, making easy to grab whether partial or full output from a task and use them as input of sub-tasks (and so on).
