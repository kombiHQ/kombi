...
class Dispatcher(object):
    ...
    def __init__(self, dispatcherType):
        ...
        if 'KOMBI_DISPATCH_TIME' not in env:
            env['KOMBI_DISPATCH_TIME'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    def dispatch(self, taskOrtaskHolder, elements=[]):
        """
        Run the dispatcher.

        Return a list of ids created by the dispatcher that can be used to track
        the dispatched task or task holder.
        """
        taskHolder = taskOrtaskHolder
        if isinstance(taskOrtaskHolder, Task):
            taskHolder = TaskHolder(taskOrtaskHolder)
        ...
        clonedTaskHolder.addElements(elements)

        # in case the task does not have any elements means there is nothing
        # to be executed, returning right away.
        if len(clonedTaskHolder.task().elements()) == 0:
            return []
