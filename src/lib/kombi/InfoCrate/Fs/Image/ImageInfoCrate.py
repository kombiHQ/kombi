...
class ImageInfoCrate(FileInfoCrate):
    """
    Abstracted image infoCrate.
    """

    def __init__(self, *args, **kwargs):
        """
        Create an image infoCrate.
        """
        super(ImageInfoCrate, self).__init__(*args, **kwargs)
    ...
    def __isStandardSequence(self):
        """
        Return a boolean telling if the infoCrate contains a standard image sequence.

        The infoCrate must follow the standard seq convention: abc.0001.ext
        """
        nameParts = self.pathHolder().baseName().split(".")
    ...
    def __isAmbiguousSequence(self):
        """
        Return a boolean telling if the infoCrate contains an ambiguous image sequence.

        This infoCrate must follow the ambiguous seq convention: abc_0001.ext
        """
        nameParts = self.pathHolder().baseName().split(".")
