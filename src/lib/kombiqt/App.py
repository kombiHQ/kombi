...
class App(QtWidgets.QApplication):
    ...
    Example:
        kombi <CONFIGURATION-DIRECTORY> [<INPUT-FILES-DIRECTORY>]
    """

    __viewModes = ["group", "flat"]
    __runOnTheFarm = os.environ.get('KOMBIAPP_RUN_FARM', '0')
    ...
    def __init__(self, argv, **kwargs):
        ...
        # otherwise from the environment
            ...
            self.__configurationDirectory = os.environ['KOMBIAPP_CONFIG_DIR']

        if self.updateConfiguration():
            self.__main.setWindowTitle('Kombi ({0})'.format(self.__configurationDirectory))
    ...
    def __buildWidgets(self):
        ...
        self.__main.setWindowTitle('Kombi')
        self.__main.resize(1080, 720)
        self.__main.setWindowIcon(
            Resource.icon("icons/kombi.png")
        )
