import os
from kombi.Crawler.Fs.Image.OiioCrawler import OiioCrawler
...
class ImageCrawlerViewer(QtWidgets.QLabel):
    """
    Basic image crawler widget.
    """

    def __init__(self, imageCrawlers, width=640, height=480):
        """
        Create an image crawler widget.
        """
        super(ImageCrawlerViewer, self).__init__()
        ...
        self.setCrawlers(imageCrawlers)
        self.__reset()
    ...
    def setCrawlers(self, imageCrawlers):
        self.__imageCrawlers = imageCrawlers
        self.__update()

    def __update(self):
        if self.__imageCrawlers:
            self.__slider.setMaximum(len(self.__imageCrawlers) - 1)
    ...
    def __finishedLoad(self, fullPath, qimage, spec):
        ...
        self.__slider.setVisible(len(self.__imageCrawlers) > 1)
        self.__currentFileLabel.setVisible(len(self.__imageCrawlers))

    def __onSliderChange(self, value):
        if not self.__imageCrawlers:
            self.__reset()
            return

        crawler = self.__imageCrawlers[value]
        self.__loadImageThread.setImageFullPath(crawler.var('fullPath'), self.width(), self.height())
