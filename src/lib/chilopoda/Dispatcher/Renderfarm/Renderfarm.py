...
class Renderfarm(Dispatcher):
    ...
    __defaultJobTempDir = os.environ.get('CHILOPODA_TEMP_REMOTE_DIR', '')
    __defaultLabel = "chilopoda"
    __defaultExpandOnTheFarm = False
    __defaultChunkifyOnTheFarm = False
    __defaultPriority = int(os.environ.get('CHILOPODA_DISPATCHER_RENDERFARM_PRIORITY', 50))
    __defaultSplitSize = int(os.environ.get('CHILOPODA_DISPATCHER_RENDERFARM_SPLITSIZE', 5))
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert len(self.__defaultJobTempDir), "CHILOPODA_TEMP_REMOTE_DIR env is not defined!"
    ...
    def __createJobDirectory(self):
        ...
        baseRemoteTemporaryPath = os.path.join(
            ...
            os.environ.get("CHILOPODA_USER", getpass.getuser()),
            str(uuid.uuid4())
        )
