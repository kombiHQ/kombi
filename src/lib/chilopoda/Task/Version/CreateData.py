...
class CreateDataFileNotUnderDataDirectoryError(TaskError):
    """Create Data File Not Under Data Directory Error."""

class CreateDataMetadataNotFoundError(TaskError):
    """Create Data Metadata Not Found Error."""

class CreateDataInvalidInfoError(TaskError):
...
class CreateData(Task):
    ...
    def configDirectory(self):
        """
        Return the path about the location for the configuration used by chilopoda.
        """
    ...
    def info(self, key):
        ...
        if key not in self.__info:
            raise CreateDataInvalidInfoError(
    ...
    def addFile(self, filePath, metadata=None):
        ...
        if not filePath.startswith(self.dataPath() + os.sep):
            raise CreateDataFileNotUnderDataDirectoryError(
    ...
    def fileMetadata(self, filePath):
        ...
        raise CreateDataMetadataNotFoundError(
    ...
    def updateInfo(self):
        ...
        self.addInfo('user', os.environ.get("CHILOPODA_USER", getpass.getuser()))
    ...
    def __copyChilopodaConfig(self):
        """
        Copy the configuration used by chilopoda to the current version.
        """
        configDirectory = os.path.join(self.rootPath(), "chilopodaConfig")
